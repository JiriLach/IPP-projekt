...
function main(): void {
  ...
  logger.info("Starting test discovery in directory: %s", args.tests_dir);

  const testFiles = findTestFiles(args.tests_dir, args.recursive);
  logger.info("Discovered %d test files.", testFiles.length);
  const discovered_test_cases: InternalTestCaseDefinition[] = [];
  const unexecuted: Record<string, UnexecutedReason> = {};

  for (const file of testFiles) {
    const parsed = parseTestFile(file);
    if (parsed instanceof UnexecutedReason) {
      unexecuted[basename(file, ".test")] = parsed;
    } else {
      discovered_test_cases.push(parsed);
    }
  }

  const { toRun, filteredOut } = filterTestCases(discovered_test_cases, args);
  filteredOut.forEach((t) => {
    unexecuted[t.name] = new UnexecutedReason(UnexecutedReasonCode.FILTERED_OUT);
  });
  ...
  if (!args.dry_run) {
    for (const test of toRun) {
      logger.info(`Spouštím test: ${test.name} [${test.category}]`);
      const report = executeTest(test);

      let catData = categories[test.category];
      if (catData === undefined) {
        catData = { total: 0, passed: 0, results: {} };
        categories[test.category] = catData;
      }

      catData.total += test.points;
      if (report.result === TestResult.PASSED) {
        catData.passed += test.points;
      }

      catData.results[test.name] = report;
    }
  }

  const finalResults: Record<string, CategoryReport> = {};
  for (const [catName, catData] of Object.entries(categories)) {
    finalResults[catName] = new CategoryReport(catData.total, catData.passed, catData.results);
  }

  const report = new TestReport({
    discovered_test_cases: discovered_test_cases,
    unexecuted: unexecuted,
    results: args.dry_run ? null : finalResults,
  });
